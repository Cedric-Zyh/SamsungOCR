    ...
    if (reviewState.artifactTab === 'date') {
      ...
      target.innerHTML = artifacts.map(item => {
        ...
        const orientationInfo = item.orientation
          ? `<article class="artifact-group"><h4>矩形印章方向 <small>${escapeHtml(item.orientation.status)} · ${percent(item.orientation.confidence)}</small></h4><div class="artifact-grid">${artifactCard('方向校正前原图', item.orientation_original_url)}${item.orientation.applied_rotation === 180 ? artifactCard('自动旋转 180° 后（后续 OCR 输入区域）', item.orientation_corrected_url) : ''}</div></article>` : '';
