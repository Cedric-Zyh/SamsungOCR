...
`205/301`（68.11%）且 `205/205` 正确；印章严格结论和可靠自动印章分别为
`235/301`（78.07%）和 `234/301`（77.74%），且可靠自动结论 `234/234`
正确。完整测试为 `498 passed`。仓库货物接收委托书已按当前使用范围排除，既不
参与标准回单准确率分母，也不作为后续优化或验收阻断项。

第七十六梯队新增“残缺年份 + 缺月份跨槽位共识”。它只处理全部严格完整日期为空
的 Hybrid 样本：Mobile 与 Server 必须在紧/宽日期行共同保留同一个明确日数字，
Mobile 宽月份上下文槽和 Server 月份数字窄槽又必须在最大通道、去横线两种预处理
共四个单元全部读到同一月份。年份只能由制单日期、运单号日期和要求到货三项独立
业务日期一致确认；候选还必须等于要求到货且处于制单/运单日期后三天内。301 张
安全矩阵预筛 3 张、只接受 `7285996828.jpg`，正确 1、误接受 0；两张控制样本
分别因日数字不等于要求日期、月份读数冲突继续待复核。真实 Hybrid 重跑将目标日期
`2025-05-01` 从 25% 提升为 84% 可靠匹配，印章因内容和视觉证据不足仍待复核，
没有用日期提升掩盖印章不确定性。界面显示日期原图、处理图、月份/日期槽位图和逐路
OCR 原文，日期 43/43 张、印章 9/9 张图片加载正常。Excel 为 3 行、16 列、2 张
工作表，中文、长订单号文本、日期/时间、百分比、状态色、渲染和公式错误扫描通过。
报告见 `storage/date-partial-year-missing-month-matrix-tier76.json`、
`storage/e2e-date-partial-year-missing-month-tier76.json`、
`storage/accuracy-report-301-hybrid-tier76.json`、
`storage/date-gap-analysis-tier76.json`、`storage/seal-gap-analysis-tier76.json`
和 `storage/windows-smoke-tier76.json`。
