...
`219/301`（72.76%；有日期样本检出 `221/300`，73.67%），其中可靠自动日期
`199/301`（66.11%）且 `199/199` 正确；印章严格结论和可靠自动印章分别为
`222/301`（73.75%）和 `221/301`（73.42%），且可靠自动结论 `221/221`
正确。完整测试为 `422 passed`。
...
也可以配置 `SEAL_API_URL` 和 `SEAL_API_TIMEOUT`。未配置密钥不会报错，界面会明确显示“本地 Vision”。
